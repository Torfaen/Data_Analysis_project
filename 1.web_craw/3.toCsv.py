...
def main():
#-----------toCsv部分-------------------------
    html_path=os.path.join("html_data",keyword)
    csv_path=os.path.join("csv_data",keyword)
    output_file_name=f"{keyword}_search_result.csv"
    path=os.path.join(csv_path, output_file_name)
    os.makedirs(os.path.join(csv_path), exist_ok=True)
